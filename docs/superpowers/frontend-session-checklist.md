...
步骤：
1. 清空浏览器该站点的 localStorage 和 sessionStorage。
2. 以游客身份打开下载页，提交一个下载时间足够长的视频。
3. 确认任务进入「下载中」后关闭标签页。
4. 等待 30 秒以上，再检查任务状态、日志和 `temp_guest` 目录。

预期：
- 30 秒内如果刷新或同一 session 重连，不应取消下载。
- 30 秒后如果没有重连，应取消该 guest session 的活跃下载。
- 临时视频、分离音频、`.part`、`.ytdl`、`.temp` 等残留应被清理。
- 其他 guest session 或正式视频库文件不应被误删。
...
2. 提交一个下载时间足够长的视频。
3. 确认任务进入「下载中」后关闭标签页。
4. 等待下载完成后重新打开下载页或用户视频库。

预期：
- 关闭页面不会取消登录用户的后台下载。
- 下载完成后视频进入普通用户 A 的视频库。
- 重新打开页面后，不应出现其他用户或游客的任务卡。
- 如果下载最终失败，失败状态应属于普通用户 A 的任务，不应污染游客 session。
...
预期：
- 前端应提示当前存在活跃下载。
- 选择取消下载并退出时，任务进入「已取消」，下载残留被清理，退出后任务列表清空。
- 选择继续后台下载并退出时，退出登录成功，当前页面切换为新的未登录 client。
- 继续后台下载完成后，普通用户任务应仍按原用户入库；管理员任务不应出现在未登录下载页。
- 再登录其他账号时，不应看到前一个账号的下载任务或视频库。
